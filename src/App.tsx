  ...
  return <div className="app-shell">
    <main className="page">
      ...
      {loadError ? <div className="card error-message" role="alert">{loadError}<button className="primary-button" onClick={() => void reload()}>重新读取</button></div> : !ledger ? <p className="empty-copy" role="status">正在打开资金账本…</p> : <>
        ...
        {(tab === 'home' || tab === 'assets') && <>
          <section className="balance-card" aria-label="当前总资产"><p className="eyebrow">{tab === 'home' ? '我的总资产' : '当前资产合计'} · {currencyName(currency)}</p><strong className={`balance-value money${(total ?? 0) < 0 ? ' negative' : ''}`} data-testid="total">{money(total, currency, tab === 'home' ? 0 : undefined)}</strong><p className="balance-caption">两种币种合并折算 · 含负债</p><div className="native-totals"><div><span>日元资产</span><b className="money">{money(totals.JPY, 'JPY')}</b></div><div><span>人民币资产</span><b className="money">{money(totals.CNY, 'CNY', tab === 'home' ? 0 : undefined)}</b></div></div></section>

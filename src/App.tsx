...
export default function App() {
  ...
  async function snapshot() { try { await mutate(value => recordSnapshot(value, value.assets, rate)); setMessage('已按当前余额与汇率保存今日快照。') } catch (error) { setMessage(errorText(error)) } }
  ...
  return <div className="app-shell">
    <main className="page">
      <header className="app-header"><div className="brand"><img src={`${import.meta.env.BASE_URL}pwa-192x192.png`} alt="" /><div><strong>资金账本</strong><small>把每一份积累，认真收好</small></div></div><span className="local-badge">本地保存</span></header>
      ...
      {loadError ? <div className="card error-message" role="alert">{loadError}<button className="primary-button" onClick={() => void reload()}>重新读取</button></div> : !ledger ? <p className="empty-copy" role="status">正在打开资金账本…</p> : <>
        ...
        {(tab === 'home' || tab === 'assets') && <>
          ...
          {tab === 'assets' && <label className="search"><Icon name="search" size={18} /><input aria-label="搜索资产来源" placeholder="搜索资产来源" value={search} onChange={event => setSearch(event.target.value)} /></label>}
          <div className="section-heading"><h2>{tab === 'home' ? '我的资产来源' : '资产明细'} <small>{ledger.assets.length} 项</small></h2>{tab === 'home' && ledger.assets.length > 5 && <button className="text-button" onClick={() => setTab('assets')}>查看全部</button>}</div>
          <section className="card assets-list">{visibleAssets.length ? assetRows(tab === 'home' ? visibleAssets.slice(0, 5) : visibleAssets) : <div className="empty"><img src={happyCat} alt="" /><h2>{search ? '没有找到这个来源' : '从第一份资产开始'}</h2><p>{search ? '试试其他关键词。' : '点击右下角猫咪，填写来源和当前余额。'}</p>{!search && <button className="text-button" onClick={() => edit()}>记一笔资产</button>}</div>}</section>
          {tab === 'home' && <div className="history-summary"><div><strong>今天的资产，留给未来看</strong><p>{ledger.snapshots.at(-1)?.day === today ? '今天的快照已保存；编辑资产会更新今日快照。' : '没有更新的日期，沿用上次保存的资产总额。'}</p></div><button className="text-button" disabled={busy || !ledger.snapshots.length} onClick={() => void snapshot()}>保存今日快照</button></div>}
        </>}
        {tab === 'history' && <History ledger={ledger} currency={currency} today={today} onEdit={editHistorical} />}
        {tab === 'settings' && <>
          <div className="section-heading"><div><p className="eyebrow">安心收好每一份记录</p><h1>设置</h1></div></div>
          ...
          <h2 className="settings-heading">数据管理</h2><section className="card settings-list"><button className="negative" disabled={busy || !ledger.snapshots.length} onClick={() => void clear()}><Icon name="trash" /><span>清空全部资产及历史<small>此操作需要两次确认</small></span></button></section><p className="app-info">资金账本 · 1.0.0<br />日元 / 人民币 · 本地保存</p>
        </>}
      </>}
    </main>

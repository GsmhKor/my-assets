...
export function History({ ledger, currency, today, onEdit }: { ledger: Ledger; currency: Currency; today: string; onEdit: (asset: Asset) => void }) {
  ...
  const valid = [...amounts, ...nativeAmounts].filter((value): value is number => value !== null)
  ...
  const path = (values: (number | null)[]) => values.map((amount, index) => amount === null ? '' : `${index === 0 || values[index - 1] === null ? 'M' : 'L'} ${x(index)} ${y(amount)}`).join(' ')
  ...
  return <>
    ...
    <section className="card history-chart"><h2>每日资产变化 · {currency}</h2>
      {points.length ? <>
        ...
        <svg viewBox="0 0 320 158" role="img" aria-label={`${month}总资产（折算）与${nativeLabel}折线图，单位 ${currency}，共用金额刻度，详细金额见下方日期列表`}>

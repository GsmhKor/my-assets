...
export function History({ ledger, currency, today, onEdit }: { ledger: Ledger; currency: Currency; today: string; onEdit: (asset: Asset) => void }) {
  ...
  return <>
    ...
    <section className="card history-chart"><h2>每日总资产 · {currency}</h2>
      {points.length ? <>
        {valid.length > 0 && <><div className="chart-labels"><span>最高 {money(max, currency, 0)}</span><span>最低 {money(min, currency, 0)}</span></div><svg viewBox="0 0 320 158" role="img" aria-label={`${month}每日总资产折线图，详细金额见下方日期列表`}><path d="M18 140H302" className="chart-axis" /><path d={path} className="chart-line" />{amounts.map((amount, index) => amount !== null && <circle key={points[index].day} cx={x(index)} cy={y(amount)} r="3" className="chart-dot" />)}</svg><div className="chart-labels"><span>{points[0].day}</span><span>{points.at(-1)?.day}</span></div></>}
    ...
    <div className="history-list">{[...points].reverse().map(point => <details className="card day-card" key={point.day}>
      <summary><span><strong>{point.day}{point.day === today ? ' · 今天' : ''}</strong><small>{point.carried ? `沿用 ${point.snapshot.day}` : '当日最后保存'}</small></span><b className="money">{money(convertedTotal(point.snapshot.totals, currency, point.snapshot.rate), currency, 0)}</b><Icon name="chevron-right" size={17} /></summary>
